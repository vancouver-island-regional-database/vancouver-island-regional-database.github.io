/* voting-page_app.js - Voting Matrix Page Logic */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // New state arrays for multi-select headers
  let headJurSelected = [];
  ...
  // 1. Dynamic controllers for table multi-selects to handle clean state resets
  let headJurController, headYeasController, headNaysController, headStatusController;

  // Sidebar categories checkboxes change listener
  ...
  // Multi-select custom dropdown builder
  ...
  // Render Dynamic Active Filter Pills (Perfect styling)
  function renderActiveFilters() {
    ...
    // 3. Voting Column/Stage/Member/Date Filters have been decoupled from the Selected Collections bar and do not render pills here.
  ...
  // Reset Filters listener with thorough console reports
  if (clearDateBtn) {
    ...
    clearDateBtn.addEventListener('click', () => {
      ...
      // Reset multiselect state closures
  ...
  // --- LOADER FOR VOTING DECISIONS WITH OFFLINE FALLBACK ---
  ...
  function processAndRenderDecisions() {
    ...
    const defaultMembersList = ["Marsh Stevens", "Deena Beeston", "Tricia McKay"];

    const yeasOptions = document.getElementById('headYeasOptions');
    if (yeasOptions) {
      yeasOptions.innerHTML = '';
      defaultMembersList.forEach(m => {
    ...
    // Initialize custom multi-select components & save references
  ...
  function applyHeaderFilters() {
    ...
    let filtered = rawDecisionsData.filter(d => {
      ...
      const defaultMembers = ["Marsh Stevens", "Deena Beeston", "Tricia McKay"];
      const yeasIsAll = !d.yeas || d.yeas.trim().toLowerCase() === 'all council';

      let matchYeaFilter = headYeasSelected.length === 0 || headYeasSelected.some(y => {
        const isDefaultMember = defaultMembers.includes(y);
        return (d.yeas && d.yeas.includes(y)) || (isDefaultMember && yeasIsAll);
      });
  ...
  function renderMatrixTable(decisions) {
    ...
    decisions.forEach(row => {
      ...
      // Clean up motion summary: strip leading comma/as-amended artifacts from parsing
      ...
      // Strip embedded role titles (legacy format: "Mayor Aaron Stone Councillor Amanda Jacobson")
      const cleanMemberList = (raw) => {
        if (!raw) return [];
        // Normalize: replace role titles used as separators with commas
        const cleaned = raw.replace(/\b(Mayor|Councillor|Councilor|Alderman|Alderperson|Deputy Mayor)\b/gi, ',');
        return cleaned.split(/[,;]/).map(s => s.trim()).filter(s => s && !unrecordedValues.includes(s.toLowerCase()));
      };

      let yeasList = [];
      if (row.yeas && typeof row.yeas === 'string' && !['none', 'n/a', '0'].includes(row.yeas.trim().toLowerCase())) {
        if (row.yeas.trim().toLowerCase() === 'all council') {
          yeasList = [...defaultMembers];
        } else {
          yeasList = cleanMemberList(row.yeas);
        }
      } else {
        yeasList = [...defaultMembers];
      }
      ...
      let naysList = [];
      if (row.nays && typeof row.nays === 'string' && !['none', 'n/a', '0'].includes(row.nays.trim().toLowerCase())) {
        naysList = cleanMemberList(row.nays);
      }
      ...
      // ── Per-row stage display ──────────────────────────────────────────────
      // Each row = one meeting document. We show what happened in THIS meeting only.
      // "first, second and third readings" → ⏩🆕📖✍️ (fast-tracked)
      // Individual readings → their own icon. Outcome always appended.
      ...
      // If nothing detected, show ✅ or ❌ alone based on outcome
      if (!stageIcons) {
        if (isPassed)    stageIcons = '✅';
    ...
    // Bind cross-referencing resolution link
  ...
  // Click handler to close open custom table dropdown menus if clicking outside
